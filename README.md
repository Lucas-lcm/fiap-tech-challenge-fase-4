...
* Avaliação do modelo com métricas (MAE, RMSE, MAPE)
* Salvamento dos artefatos treinados (modelo `.keras` e scaler `.pkl`)

---

## Resultados e Análise

O modelo treinado foi submetido a testes com dados desconhecidos (conjunto de teste) para validar sua capacidade de generalização. A análise focou na capacidade da rede LSTM de capturar a volatilidade e a tendência do ativo NFLX.

### Métricas de Desempenho

Os testes apresentaram os seguintes resultados quantitativos:

| Métrica | Valor | Interpretação |
| --- | --- | --- |
| **MAE** | **1.1644** | O erro médio absoluto é de aprox. $1.16 USD por previsão. |
...
3. **Conclusão:** O modelo é estatisticamente robusto para ser utilizado no ambiente de produção via API.
